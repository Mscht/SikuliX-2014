...
//      return None
  public String capture(Object[] args) {
    ...
    try {
      if (args.length == 0) {
        simg = Screen.getPrimaryScreen().userCapture();
      } else if (args.length == 1) {
        if (args[0] instanceof String) {
          simg = Screen.getPrimaryScreen().userCapture((String) args[0]);
        } else {
          simg = Screen.getPrimaryScreen().capture((Region) args[0]);
        }
...
//      return None
  public String saveCapture(Object[] args) {
    ...
    try {
      if (args.length == 1) {
        fsimg = Screen.getPrimaryScreen().saveCapture((String) args[0]);
      } else if (args.length == 1) {
        fsimg = Screen.getPrimaryScreen().saveCapture((String) args[0], (Region) args[1]);
      } else if (args.length == 1) {
        fsimg = Screen.getPrimaryScreen().saveCapture((String) args[0],
...
//      return None
  public Region selectRegion(String msg) {
    ...
    if (msg == null) {
      ...
      reg = Screen.getPrimaryScreen().selectRegion(msg);
    }
    return reg;
  }
}
